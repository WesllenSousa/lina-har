...
public class BOSSMD {
    ...
    private TimeSeries[][] splitMultiDimTimeSeries(int numSources, TimeSeriesMD[] samples) {
     ...
     */
    public int[][] createMDWords(final TimeSeriesMD[] samples) {
    ...
    public BagOfPatternMD[] createBagOfPattern(
        ...
        for (int j = 0; j < mdWords.length; j++) {
            bagOfPatterns[j] = new BagOfPatternMD(mdWords[j].length, Double.parseDouble(samples[j].getLabel()));
