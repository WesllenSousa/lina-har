...
/**
 * The Momentary Fourier Transform is alternative algorithm of the Discrete
 * Fourier Transform for overlapping windows. It has a constant computational
 * complexity for in the window queryLength n as opposed to O(n log n) for the
 * Fast Fourier Transform algorithm.
 * <p>
 * It was first published in: Albrecht, S., Cumming, I., Dudas, J.: The
 * momentary fourier transformation derived from recursive matrix
 * transformations. In: Digital Signal Processing Proceedings, 1997., IEEE
 * (1997)
 *
 * @author bzcschae
 */
public class MFT implements Serializable {
    ...
    /**
     * Transforms a time series using the *discrete* fourier transform. Results
     * in a single Fourier transform of the time series.
     *
    ...
    /**
     * Transforms a time series, extracting windows and using *momentary*
     * fourier transform for each window. Results in one Fourier transform for
     * each window. Returns only the first l/2 Fourier coefficients for each
     * window.
     *
     * @param timeSeries the time series to be transformed
     ...
     * windowSize is smaller than l, only the first windowSize Fourier values
     * are set.
    ...
    /**
     * Apply normalization to the Fourier coefficients to allow lower bounding
     * in Euclidean space
     */
