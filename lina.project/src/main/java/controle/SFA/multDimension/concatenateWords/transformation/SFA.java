...
import controle.SFA.multDimension.concatenateWords.classification.Classifier.Words;
...
public class SFA implements Serializable {
     ...
     * @param wordLength Length of the resulting SFA words. Each character of a
     ...
     * word queryLength corresponds to a stronger low-pass filtering of the time
     * series.
    ...
    protected double[][] fillOrderline(TimeSeries[] samples, int l) {
        ...
        for (int i = 0; i < samples.length; i++) {
            ...
            for (int j = 0; j < transformedSamples[i].length; j++) {
                ...
                this.orderLine[j].add(new ValueLabel(value, Double.parseDouble(samples[i].getLabel())));
            }
        }
