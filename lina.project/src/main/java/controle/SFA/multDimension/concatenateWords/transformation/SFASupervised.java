...
public class SFASupervised extends SFA {
     ...
     * word corresponds to one Fourier value. As opposed to the normal SFA boss,
     * here characters correspond to those Fourier values that are most
     * distinctive between class labels.
    ...
    public static Indices<Double>[] calcBestCoefficients(
        ...
        for (int i = 0; i < samples.length; i++) {
            ...
            if (allTs == null) {
                allTs = new ArrayList<>();
                classes.put(Double.parseDouble(samples[i].getLabel()), allTs);
            }
     ...
     * the same population mean. The test is applied to samples from two or more
     * groups, possibly with differing sizes.
     *
     * @param length
     * @param classes
     * @param nSamples
