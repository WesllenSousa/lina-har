...
public abstract class MDClassifier {
    ...
    public MDClassifier(TimeSeriesMD[] train, TimeSeriesMD[] test) {
     ...
     */
    public abstract Score fit(final TimeSeriesMD[] trainSamples);
     ...
     */
    public abstract Predictions score(final TimeSeriesMD[] testSamples);
     ...
     */
    public abstract Double[] predict(final TimeSeriesMD[] testSamples);
    ...
    protected Predictions evalLabels(TimeSeriesMD[] testSamples, Double[] labels) {
        int correct = 0;
        for (int ind = 0; ind < testSamples.length; ind++) {
            correct += compareLabels(labels[ind], Double.parseDouble(testSamples[ind].getLabel())) ? 1 : 0;
        }
...
//    }
    ...
    protected int getMax(TimeSeriesMD[] samples, int MAX_WINDOW_SIZE) {
    ...
    protected static Set<Double> uniqueClassLabels(TimeSeriesMD[] ts) {
        Set<Double> labels = new HashSet<>();
        for (TimeSeriesMD t : ts) {
            labels.add(Double.parseDouble(t.getLabel()));
        }
        return labels;
    }
    ...
    protected void generateIndices(TimeSeriesMD[] samples) {
    ...
    protected IntArrayList[] getStratifiedTrainTestSplitIndices(
        ...
        for (int i = 0; i < samples.length; i++) {
            Double label = Double.parseDouble(samples[i].getLabel());
