package controle.CRF.Model;
import controle.CRF.CRF.DataSequence;
import controle.CRF.CRF.SegmentDataSequence;
...
public abstract class FeatureTypes implements Serializable {
    ...
    public int maxFeatureId() {
		controle.CRF.Utils.LogMessage.issueWarning("WARNING : Class " + getClass().getName() + " does not implement maxFeatureId(). Returning default value. Please refer to the documentation.");
    ...
    public void train(DataSequence data, int pos) {
		if(requiresTraining()) {
			controle.CRF.Utils.LogMessage.issueWarning("WARNING : Class " + getClass().getName() + " does not implement the train(DataSequence, int) method. Please implement the train() methods properly.");
		}
	}	
    /**
    ...
    public void train(SegmentDataSequence sequence, int segStart, int segEnd) {
		if(requiresTraining()) {
			controle.CRF.Utils.LogMessage.issueWarning("WARNING : Class " + getClass().getName() + " does not implement the train(SegmentDataSequence, int, int) method. Calling train(DataSequence, int) instead. Please implement the train() methods properly.");
			train(sequence, segEnd);
        }
	}	
    /**
     * @return
    ...
    public void trainingDone() {
        if(requiresTraining()) {
            controle.CRF.Utils.LogMessage.issueWarning("WARNING : Class " + getClass().getName() + " does not implement the train(SegmentDataSequence, int, int) method. Calling train(DataSequence, int) instead. Please implement the trainDone() methods properly.");
        }
    }
    public void disable() {
        disabled=true;
