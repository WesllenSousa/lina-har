...
import controle.CRF.BSegmentCRF.BSegmentTrainer.MatrixWithRange;
import controle.CRF.CRF.DataSequence;
import controle.CRF.CRF.LabelSequence;
import controle.CRF.CRF.LogSparseDoubleMatrix1D;
import controle.CRF.CRF.SegmentViterbi;
import controle.CRF.CRF.Segmentation;
import controle.CRF.CRF.Soln;
import controle.CRF.CRF.SparseViterbi;
import controle.CRF.CRF.SparseViterbi.Context;
...
public class BSegmentViterbi extends SparseViterbi {
    ...
    protected double getCorrectScore(DataSequence dataSeq, int i, int ell, double[] lambda) {
        ...
        if (segLength > m) {
            ...
            if (!reuseM) {
                fstore.getLogMi(i - segLength + 1, Mi);
            }
        }
    ...
    protected void computeLogMi(DataSequence dataSeq, int i, int ell, double lambda[]) {
        ...
        while (openRi.start != i - ell + 1) {
            ...
            if (lossAugmentedScore) {
                adjustScore(dataSeq, Ri, openRi, i, ell);
            }
        }
...
//      Ri at this point contains features for segment (i-m+1, i)
        ...
        if (i - m >= -1) {
            ...
            if (i - m == -1) {
                Ri.forEachNonZero(applyFunc.init(Ri, openContext[i]));
            }
        }
        if (i - m >= 0) {
