...
class DCTrainRecord implements TrainRecord, Segmentation {
    ...
    public int length() {
        return labelsPerToken.length;
    }

    public Object x(int i) {
        return _tokens[snum[i]][i - spos[snum[i]]];
    }

    public int y(int i) {
        return labelsPerToken[i];
    }
...
public class DataCruncher {
    ...
    /**
     * Reads a block of text ended by a blank line or the end of the file. The
     * block contains lines of tokens with a label.
     *
    ...
    static int readRowFixedCol(int numLabels, BufferedReader tin, String tagDelimit,
        ...
        for (int i = 0; (i < labels.length) && firstSplit.hasMoreTokens(); i++) {
            ...
            if (tagDelimit.indexOf(w) != -1) {
                continue;
            } else if (firstSplit.hasMoreTokens()) // read past the delimiter.
            {
     ...
     * @param numLabels The maximal number of labels expected
     * @param tin
     * @param tagDelimit A character as String that acts as a delimiter between
     * tokens and label.
     * @return An array with labels if the data are in fixed column format, null
     * otherwise.
     * @throws IOException
     */
