...
//      public FeatureVector edgeFeatureIds = new FeatureVector();
        ...
        public int add(Feature f) {
            ...
            if (distinctFeatures.get(f.index())==null) {
                ...
                return distinctFeatures.get(f.index()).add(f);
            }
        }
        ...
        public Feature get(int featureId) {
            if (featureId >= 0) {
                return distinctFeatures.get(featureId); // distinctFeatures[featureId];
            } else {
                return featureVariants.get(-1*featureId-1);
            }
        }
    ...
    class Stats {
        ...
        int[] getStartEndOffsets(int prevPos, int pos) {
            return segmentFeatureOffsets.get(getKey(prevPos,pos));
        }
        /**
         * @param data
         * @param pos
        ...
        public boolean initSegment(DataSequence data, int prevPos, int pos) {
            ...
            cacheThis=true;
            thisSegmentOffsets = segmentFeatureOffsets.get(getKey(prevPos,pos));
    ...
    class Cursor {
        ...
        public void init(DataSequence data, int prevPos, int pos) {
            int[][] tfeatures[] = perSegmentFeatureOffsets.get(getDataIndex(data));
