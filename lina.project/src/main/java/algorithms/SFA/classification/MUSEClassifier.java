...
import algorithms.SFA.transformation.MUSE;
 ...
 * <p>
 * Schäfer, P., Leser, U.: Multivariate Time Series Classification with
 * WEASEL+MUSE. arXiv 2017 http://arxiv.org/abs/1711.11343
 */
public class MUSEClassifier extends Classifier {
    ...
    public Score eval(
            MultiVariateTimeSeries[] trainSamples, MultiVariateTimeSeries[] testSamples) {
        ...
        try {
            ...
            for (final SFA.HistogramType histType : histTypes) {
                for (final boolean mean : NORMALIZATION) {
                    final MUSE model = new MUSE(maxWordLength, maxSymbol, histType, windowLengths, mean, lowerBounding);
            ...
            MUSE model = new MUSE(bestF, maxSymbol, bestHistType, windowLengths, bestNorm, lowerBounding);
    ...
    public String toString() {
        return "Name: WEASEL \n"
                ...
                + "Word: " + model.muse.maxF + "\n"
                + "Norm: " + model.muse.normMean + "\n"
                + "Symbol: " + model.muse.alphabetSize + "\n"
                + "Window: " + model.muse.windowLengths[0] + "\n";
    }
