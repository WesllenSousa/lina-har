...
public class ExpLauncher {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 2) {
            if (args[ind].toLowerCase().trim().contains("-blm")) {
                if (args[ind + 1].toLowerCase().trim().contains("train")) {
                    ...
                    if (clusOption) {
                        CWSCModel = training.buildModel(args[ind + 2], args[ind + 4], args[ind + 6]);
                    } else {
                        CWSCModel = training.buildModel(args[ind + 2], "", "");
                    }

                } else if (args[1].toLowerCase().trim().contains("load")) {
                    System.out.println("Load Model .....");
                    CWSCModel = expLauncher.readModel(args[2]);
                }
            }
        } else {
        ...
        AnyNovelLauncher launcher = new AnyNovelLauncher();
        launcher.run(CWSCModel, testName, validName);
    ...
    public BLM readModel(String s) throws Exception {
        baseModel = null;
        BLM CWSCModel = null;
    ...
    public BLM readFromDatFile(BufferedReader in) throws IOException {
    ...
    private static Instance convertStringArrtoInst(String[] strArr) {
        ...
        Instance ins = (Instance) new Instances(dataRaw);
